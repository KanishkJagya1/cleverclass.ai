...
const triggerCls = cn(
  ...
  "bg-[var(--surface-1)]/60 px-3 text-[length:var(--text-xs)] font-medium text-[color:var(--text-2)]",
  "transition-colors hover:border-[var(--border-2)] hover:text-[color:var(--text-1)]",
);
...
const itemCls = cn(
  ...
  "px-3 py-2 text-[length:var(--text-sm)] text-[color:var(--text-2)] outline-none",
  "data-[highlighted]:bg-[var(--surface-0)] data-[highlighted]:text-[color:var(--text-1)]",
);
    ...
    <DropdownMenu.Root>
      <DropdownMenu.Trigger className={triggerCls} aria-label={`Board: ${current.label}`}>
        <span className="truncate" suppressHydrationWarning>
          {(ready ? current.short : BOARDS[0]!.short).replace(" Board", "")}
        </span>
        <ChevronDown className="size-3.5 shrink-0 opacity-60" aria-hidden />
      </DropdownMenu.Trigger>
      <DropdownMenu.Portal>
        <DropdownMenu.Content className={contentCls} sideOffset={8} align="end">
          <p className="px-3 pb-1.5 pt-1 text-[length:var(--text-2xs)] font-semibold uppercase tracking-[var(--tracking-wide)] text-[color:var(--text-3)]">
            Board
          </p>
          {BOARDS.map((b) => (
            <DropdownMenu.Item key={b.id} className={itemCls} onSelect={() => setBoard(b.id)}>
              {b.label}
              {board === b.id && <Check className="size-4 text-[color:var(--brand-base)]" aria-hidden />}
            </DropdownMenu.Item>
          ))}
...
export function MediumSwitcher() {
  ...
  return (
    <DropdownMenu.Root>
      ...
      >
        <span className="truncate" suppressHydrationWarning>
          {ready && current ? current.labelNative : "All"}
        </span>
        <ChevronDown className="size-3.5 shrink-0 opacity-60" aria-hidden />
      </DropdownMenu.Trigger>
      <DropdownMenu.Portal>
        <DropdownMenu.Content className={contentCls} sideOffset={8} align="end">
          ...
          <DropdownMenu.Item className={itemCls} onSelect={() => setMedium(null)}>
            All mediums
            {medium === null && <Check className="size-4 text-[color:var(--brand-base)]" aria-hidden />}
          </DropdownMenu.Item>
          {MEDIUMS.map((m) => (
            <DropdownMenu.Item key={m.id} className={itemCls} onSelect={() => setMedium(m.id)}>
              <span>
                {m.label}
                <span className="ml-2 text-[color:var(--text-3)]" lang="mr">
                  {m.id !== "english" && m.labelNative}
                </span>
              </span>
              {medium === m.id && <Check className="size-4 text-[color:var(--brand-base)]" aria-hidden />}
            </DropdownMenu.Item>
          ))}
        </DropdownMenu.Content>
      </DropdownMenu.Portal>
